...
def analyze_prototype(
    h0: np.ndarray, fs: float, crossover_hz: float, transition_hz: float
) -> Tuple[float, float, float]:
    f, H = _freq_response(h0, fs)
    mag_db = db20(H)

    # Use actual target band edges instead of static percentages
    pb_end = crossover_hz - (transition_hz / 2.0)
    sb_start = crossover_hz + (transition_hz / 2.0)
...
def _design_for_target(
    ...
    for w in probe_weights:
        ...
        slope, stopband_db, _ = analyze_prototype(h, fs, crossover_hz, transition_hz)
    ...
    try:
        ...
        _, sb_lo, _ = analyze_prototype(h_lo, fs, crossover_hz, transition_hz)
    except Exception:
        sb_lo = -float("inf")
        
    try:
        h_hi = _design_with_weight(fs, crossover_hz, taps, transition_hz, hi_w)
        _, sb_hi, _ = analyze_prototype(h_hi, fs, crossover_hz, transition_hz)
    except Exception:
        sb_hi = -float("inf")
    ...
    if np.isfinite(sb_lo) and np.isfinite(sb_hi) and sb_lo <= target_stopband_db <= sb_hi:
        ...
        for _ in range(iterations):
            ...
            _, sb, _ = analyze_prototype(h, fs, crossover_hz, transition_hz)
...
def search_best_design(
    ...
    for taps in range(min_taps, max_taps + 1, 2):
        print(f"\rEvaluating permutations for {taps}/{max_taps} taps...", end="", flush=True)
        for transition_hz in _candidate_transitions(fs, crossover_hz):
            ...
            h0, weight = pair
            slope, achieved_stopband_db, pb_ripple_db = analyze_prototype(h0, fs, crossover_hz, float(transition_hz))
...
def main() -> None:
    ...
    print(f"\n✅ Wrote: {args.output}")
    print(
        f"Filters Details: Taps = {result.taps}, Transition = {result.transition_hz:.3f} Hz, "
